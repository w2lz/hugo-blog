...
答：第一个选项是，MySQL 读出数据，发现值与原来相同，不更新，直接返回，执行结束。这里可以用一个锁实验来确认。假设，当前表 t 里的值是 (1,2)。
...
![可见性验证方式](https://file.yingnan.wang/mysql/MySQL%E5%AE%9E%E6%88%9845%E8%AE%B2/441682b64a3f5dd50f35b12ca4b87c96.webp)
...
其实 MySQL 是确认了的。只是在这个语句里面，MySQL 认为读出来的值，只有一个确定的 (id=1), 而要写的是 (a=3)，只从这两个信息是看不出来“不需要修改”的。作为验证，可以看一下下面这个例子。

![可见性验证方式 -- 对照](https://file.yingnan.wang/mysql/MySQL%E5%AE%9E%E6%88%9845%E8%AE%B2/63dd6df32dacdb827d256e5acb9837c1.webp)
